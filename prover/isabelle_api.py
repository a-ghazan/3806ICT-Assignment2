...
def _body_get(body: Any, name: str, default: Any = None) -> Any:
    """
    Read a field from either:
    - a dict-like response body, or
    - an object-style response body returned by newer/different isabelle_client versions.
    """
    if isinstance(body, dict):
...
def finished_ok(resps: List[IsabelleResponse]) -> Tuple[bool, Dict[str, Any]]:
    """
    Return success if any FINISHED response reports success.

    Why this needs to be robust:
    ----------------------------
    Different isabelle_client versions return response bodies in different shapes.
    Some return JSON/dict-style bodies, while others return Python objects with
    attributes such as `.ok`, `.result`, `.errors`, or `.nodes`.

    Earlier versions of this function only handled dict/JSON bodies. That caused
    valid Isabelle proof attempts such as `by simp` or `apply simp ... sorry` to
    be treated as failures when the response body was object-style.

    This matters because the whole proof search depends on this function:
    if finished_ok incorrectly returns False, the prover discards good tactics and
    the beam dies immediately.
    """
    ...
    for r in (resps or []):
        ...
        # Case 1: dict / JSON body
        if isinstance(obj, dict):
            last_obj = obj
            ok_value = obj.get("ok", False)
            result_value = obj.get("result", "")
            timeout_value = obj.get("timeout", "")
        else:
            # Case 2: object-style body, e.g. UseTheoriesResult(...)
            ok_value = _body_get(raw_body, "ok", False)
            result_value = _body_get(raw_body, "result", "")
            timeout_value = _body_get(raw_body, "timeout", "")

            # Keep a small debug-friendly summary rather than trying to JSON-ify the object.
            last_obj = {
        ...
        if bool(ok_value) or str(result_value).lower() == "ok":
            if str(timeout_value).lower() in ("1", "true", "yes"):
                continue
            any_ok = True
